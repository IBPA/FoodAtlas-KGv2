...
def add_mesh_ids_to_fa_chemicals(
        ...
        pubchem_to_ctd: dict) -> pd.DataFrame:
    """
    ...
    """
    fa_chemicals.loc[fa_chemicals[ENTITY_TYPE] == "chemical", EXTERNAL_IDS] = fa_chemicals.loc[fa_chemicals[ENTITY_TYPE] == "chemical", EXTERNAL_IDS].apply(
        lambda x: {**x, MESH_IDS: pubchem_to_ctd[x[PUBCHEM_CID]]} if (PUBCHEM_CID in x and x[PUBCHEM_CID] in pubchem_to_ctd) else x)
    return fa_chemicals
...
def get_max_fa_id(fa_entities: pd.DataFrame) -> int:
    ...
    """
    fa_entities[FA_ID + "_int"] = fa_entities[FA_ID].str.extract(
        r'(\d+)').astype(int)
...
def filter_ctd_chemdis(
    ...
    mesh_ids = [
        mesh_id for mesh_ids in pubchem_to_ctd.values() for mesh_id in mesh_ids]
    mesh_ids_set = set(mesh_ids)
    ctd_chemdis = ctd_chemdis[ctd_chemdis[CTD_CHEMICAL_ID].isin(
        mesh_ids_set) | ctd_chemdis[CTD_CHEMICAL_NAME].str.lower().isin(fa_chem_lookup.keys())].reset_index(drop=True)
    return ctd_chemdis


def get_disease_ids_from_alt_disease_ids(
        row: pd.Series
) -> dict:
    """
    ...
    alt_disease_ids = [{item[0]: ":".join(item[1:])}
    ...
    alt_disease_ids_full = {CTD_ALTID_MAPPING[key]: value
                            for key, value in alt_disease_ids_full.items()}
    # change values to int if possible
    for key, value in alt_disease_ids_full.items():
        alt_disease_ids_full[key] = [int(x) if x.isdigit() else x
                                     for x in value]
...
def create_disease_entities(
    ...
    ctd_diseases = ctd_diseases[ctd_diseases[CTD_DISEASE_ID].isin(
        ctd_chemdis[CTD_DISEASE_ID])].reset_index(drop=True)
    ...
    fa_entities = pd.concat([fa_entities, ctd_diseases],
    ...
    for i, row in fa_entities.iterrows():
        if MESH_IDS in row[EXTERNAL_IDS]:
            if isinstance(row[EXTERNAL_IDS][MESH_IDS], list):
                ...
                if logger:
                    logger.error(
                        f"mesh_ids is not a list for {row[FA_ID]}")
        ...
        if CTD_ALTID_MAPPING["OMIM"] in row[EXTERNAL_IDS]:
            if isinstance(row[EXTERNAL_IDS][CTD_ALTID_MAPPING["OMIM"]], list):
                ...
                if logger:
                    logger.error(
                        f"omim_ids is not a list for {row[FA_ID]}")
...
def create_disease_triplets_metadata(
    ...
    """
    # create mapping from CTD ChemicalID to FoodAtlas entity id
    fa_entities_chemicals = fa_entities[fa_entities[ENTITY_TYPE] == "chemical"].reset_index(
        drop=True)
    ...
    fa_entities_diseases = fa_entities[fa_entities[ENTITY_TYPE] == "disease"].reset_index(
        drop=True)
    ...
    ctd_chemdis_metadata[FA_ID] = ctd_chemdis["metadata_ids"].apply(
        lambda x: x[0])
    ctd_chemdis_metadata["source"] = "ctd"
    pubmed_to_pmcid = load_pmid_to_pmcid_mapping()
    pubmed_to_pmcid = pubmed_to_pmcid.set_index(
        PUBMED_IDS)[PMCID].to_dict()
...
def main(
    ...
    pubchem_to_ctd = pubchem_to_ctd.set_index(
        PUBCHEM_CID)[CTD_CHEMICAL_ID].to_dict()
    ...
    fa_entities.to_csv(os.path.join(
        kg_dir, "entities_meshids_tmp.tsv"), sep='\t', index=False)
    ...
    # save entities
    fa_entities.to_csv(os.path.join(
        kg_dir, "entities.tsv"), sep='\t', index=False)
    ...
    fa_triplets, fa_metadata = create_disease_triplets_metadata(
        ...
        fa_chem_lookup, logger, chem_id_pubchem_name_to_fa_id_not_match_file)
    # save triplets and metadata
    fa_triplets.to_csv(os.path.join(
        kg_dir, "triplets.tsv"), sep='\t', index=False)
    fa_metadata.to_csv(os.path.join(
        kg_dir, "metadata_diseases.tsv"), sep='\t', index=False)
