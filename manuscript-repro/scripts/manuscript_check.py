...
def _core_graph_metrics() -> dict[str, Any]:
    """Scale checks against this repo's KG export (`outputs/kg/`)."""
    entities_path = kg_dir() / "entities.tsv"
    triplets_path = kg_dir() / "triplets.tsv"
    if not entities_path.exists() or not triplets_path.exists():
        return {
            "entity_counts": {},
            "relation_counts": {},
            "chem_disease_total": 0,
            "kg_missing": True,
        }

    entities = pd.read_csv(entities_path, sep="\t")
    triplets = pd.read_csv(triplets_path, sep="\t")
    if "foodatlas_id" not in entities.columns or "entity_type" not in entities.columns:
        raise ValueError(f"Unexpected entities.tsv schema in {entities_path}")
    if not {"head_id", "tail_id", "relationship_id"}.issubset(triplets.columns):
        raise ValueError(f"Unexpected triplets.tsv schema in {triplets_path}")
...
def _substitution_metrics() -> dict[str, Any]:
    base = ANALYSIS / "substitutions" / "intermediate"
    out: dict[str, Any] = {}
    if not base.is_dir():
        for mode in ["disease", "bioactivity"]:
            out[mode] = {
                ...
                "by_meal": {
                    m: {"n": 0, "mean": float("nan")} for m in ["breakfast", "lunch", "dinner"]
                },
            }
        return out
    ...
    for mode in ["disease", "bioactivity"]:
        ...
        for meal in ["breakfast", "lunch", "dinner"]:
            pkl = base / mode / meal / "substitutions.pkl"
            if not pkl.exists():
                meal_metrics[meal] = {"n": 0, "mean": float("nan")}
                continue
...
def run_checks() -> tuple[list[CheckResult], dict[str, Any]]:
    ...
    kg_missing = core.get("kg_missing", False)

    # Claim 1: data variety / scale (from outputs/kg)
    if kg_missing:
        results.append(
            CheckResult(
                claim="sources_scale",
                metric="kg_files",
                manuscript_value="entities.tsv + triplets.tsv",
                observed_value=None,
                status="missing",
                note=f"Expected {kg_dir()}/entities.tsv and triplets.tsv (place exports under outputs/kg/ or see repo README).",
            )
        )
    else:
        results.extend(
    ...
    if not portfolio_text.strip():
        results.append(
            CheckResult(
                ...
                metric="portfolio_report",
                manuscript_value="present",
                observed_value="missing",
                status="missing",
                note=f"Add pinned portfolio at {portfolio_path} (see analysis_outputs/README.md).",
            )
        )
    else:
    ...
    subs_missing = not (ANALYSIS / "substitutions" / "intermediate").is_dir()
    if subs_missing:
        results.append(
            CheckResult(
                claim="substitutions",
                metric="substitution_pickles",
                manuscript_value="present",
                observed_value="missing",
                status="missing",
                note=f"Populate {ANALYSIS}/substitutions/intermediate/... (see analysis_outputs/README.md).",
            )
        )
    else:
    ...
    context = {
        "paths": {
            ...
            "kg_dir": str(kg_dir()),
            "analysis_artifacts_root": str(ANALYSIS),
        },
        "pinned_runs": {
...
def _to_markdown(results: list[CheckResult], context: dict[str, Any]) -> str:
    ...
    lines.append("## Paths")
    lines.append("")
    for k, v in context["paths"].items():
        lines.append(f"- `{k}`: `{v}`")
    lines.append("")
