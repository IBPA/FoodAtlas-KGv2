...
- [`manuscript-repro`](./manuscript-repro): Manuscript reproduction helpers (metric checks against `outputs/kg/`, optional `analysis_outputs/` for downstream figures). **Self-contained** — no references to directories outside this repository. Run **`make revalidate`** when **`outputs/kg/`** already contains graph TSVs (copied in or from an earlier pipeline run) to assert Fig. 2 scale metrics vs the paper; optional tests: `cd manuscript-repro && PYTHONPATH=scripts python3 -m unittest discover -s tests -v`.
