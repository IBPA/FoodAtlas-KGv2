...
def print_stats_with_connections():
    ...
    triplets_r1 = triplets[
        (triplets["relationship_id"] == "r1")
        & (triplets["head_id"].isin(foods_with_connections.index))
    ]
    ...
    n_triplets_r2 = dfs(food_ontologies, foods_with_connections.index) + dfs(
        chemical_ontologies, list(chemicals)
    )
    triplets_r3 = triplets[
        (triplets["relationship_id"] == "r3") & (triplets["head_id"].isin(chemicals))
    ]
    triplets_r4 = triplets[
        (triplets["relationship_id"] == "r4") & (triplets["head_id"].isin(chemicals))
    ]
    triplets_r5 = triplets[
        (triplets["relationship_id"] == "r5") & (triplets["head_id"].isin(chemicals))
    ]
    ...
    total = (
        len(triplets_r1) + n_triplets_r2 + len(triplets_r3) + len(triplets_r4)
        + len(triplets_r5)
    )
    print(f"Total: {total}")
...
def print_entities_db_coverage():
    ...
    print(
        chemical_ids.notnull().sum(), chemical_ids.notnull().sum() / len(chemical_ids)
    )

    # Disease entities.
    triplets_r3 = triplets[triplets["relationship_id"] == "r3"]
    triplets_r4 = triplets[triplets["relationship_id"] == "r4"]
    disease_ids = set(triplets_r3["tail_id"].unique()) & set(
        triplets_r4["tail_id"].unique()
    )
    ...
    # Flavor entities.
    triplets_r5 = triplets[
        (triplets["relationship_id"] == "r5")
        & (triplets["head_id"].isin(chemical_entities.index))
    ]
