...
if __name__ == "__main__":
    ...
    entities = pd.read_csv("outputs/kg/entities.tsv", sep="\t").set_index(
        "foodatlas_id"
    )
    ...
    flavor_chemicals = chemicals[
        chemicals.index.isin(triplets_flavor["head_id"].unique())
    ]
    flavor_cids = (
        flavor_chemicals["external_ids"]
        .apply(lambda x: literal_eval(x)["pubchem_compound"][0])
        .unique()
        .tolist()
    )
    ...
    for i, row in fg_foods.iterrows():
        ...
        if food_name in lut_foods.index:
            if lut_foods.loc[food_name]["foodatlas_id"][0] in foods.index:
                fg_foods.loc[i]["matched_eids"].append(
                    lut_foods.loc[food_name]["foodatlas_id"][0]
                )
                n_found += 1
                hit = True
        else:
    ...
    print(
        f"# Flavor Chemical id hits: {n_found} "
        f"({n_found / len(fg_flavor_chemicals):.4f})"
    )
