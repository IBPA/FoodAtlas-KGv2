...
def translate_greek_letter(name):
    greek2eng = {
        'α': 'alpha',
        'β': 'beta',
        'γ': 'gamma',
    ...
    greek_letters = greek2eng.keys()
    for gl in greek_letters:
        if gl in name:
            print(name)

names_chemical = pd.read_csv(
    "outputs/kg/20240214102617/_names_not_in_lut_chemical.txt",
    header=None,
    names=['name'],
    sep='\t',
)
names_chemical['name'] = names_chemical['name'].apply(translate_greek_letter)
print(names_chemical)
# names_pies = pd.read_csv(
#     "outputs/kg/20240214102617/_query_pies.txt",
#     header=None,
