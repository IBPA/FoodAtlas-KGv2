...
def main(
    ...
    data_raw_with_not_parsed = data_raw[
        data_raw["tuples_not_parsed"].apply(lambda x: True if len(x) != 0 else False)
    ]
    ...
    lut_food = dict(zip(lut_food["name"], lut_food["foodatlas_id"], strict=False))
    lut_chemical = pd.read_csv("outputs/kg/lookup_table_chemical.tsv", sep="\t")
    lut_chemical = dict(
        zip(lut_chemical["name"], lut_chemical["foodatlas_id"], strict=False)
    )
    ...
    print(
        f"# chemical name hits: {chemical_name_hits.sum()} "
        f"({chemical_name_hit_rate:.4f})"
    )
    ...
    unique_chemical_name_hit_rate = sum(unique_chemical_name_hits) / len(
    ...
    print(
        f"# unique food name hits: {sum(unique_food_name_hits)} "
        f"({unique_food_name_hit_rate:.4f})"
    )
    print(
        f"# unique chemical name hits: {sum(unique_chemical_name_hits)} "
        f"({unique_chemical_name_hit_rate:.4f})"
    )
