...
def load_lookup_tables(path_kg: str = "outputs/kg") -> list[dict]:
    ...
    if food_names:
        ...
        for i, name in enumerate(tqdm(food_names)):
            ...
            if i % 100 == 99 or i == len(food_names) - 1:
                ...
                records_search_new["search_term"] = food_names[
                    i - (len(records_search_new_rows) - 1) : i + 1
                ]
...
def query_ncbi_taxonomy(
    ...
    ncbi_taxon_ids = list(
        set([item for items in records_search["IdList"].tolist() for item in items])
    )
...
def query_pubchem_compound(
    ...
    chemical_names_new = [
        x for x in tqdm(chemical_names) if x not in chemical_names_searched_set
    ]
    ...
    cids = (
        chemical_names_searched.query(f"name in {chemical_names}")["cid"]
        .dropna()
        .unique()
        .tolist()
    )
    ...
    if os.path.exists(f"{path_cache_dir}/_cached_fetch_pubchem_compound.tsv"):
        ...
        cids_not_fetched = [x for x in cids_new if x not in cids_fetched]
    else:
    ...
    records_fetch = records_fetch.query(f"CID in {cids_new}")
