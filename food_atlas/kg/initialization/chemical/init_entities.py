    ...
    for _, row in cdno.iterrows():
        ...
        if chebi_id in chebi2fa:
            ...
            if row["fdc_nutrient_ids"]:
                kg.entities._entities.at[fa_id, "external_ids"]["fdc_nutrient"] = row[
                    "fdc_nutrient_ids"
                ]
            n_linked += 1
        else:
...
def _append_entities_from_fdc(kg) -> KnowledgeGraph:
    ...
    for _, row in fdc.iterrows():
        if row.name in fdc2fa:
            ...
            kg.entities._entities.at[fa_id, "external_ids"]["fdc_nutrient"] = [row.name]
            n_linked += 1
        else:
            entities_not_added.append(row)
    ...
    def add_placeholder_from(row):
        if "_placeholder_to" in row["external_ids"]:
            for fa_id in row["external_ids"]["_placeholder_to"]:
                if (
                    "_placeholder_from"
                    not in kg.entities._entities.at[fa_id, "external_ids"]
                ):
                    kg.entities._entities.at[fa_id, "external_ids"][
                        "_placeholder_from"
                    ] = []
                kg.entities._entities.at[fa_id, "external_ids"][
                    "_placeholder_from"
