...
def heads_tails_exist(entities, triplets):
    assert((entities[entities['foodatlas_id'].isin(triplets[triplets['relationship_id'] == 'r5']['head_id'])]['entity_type'] == 'chemical').all())
    assert((entities[entities['foodatlas_id'].isin(triplets[triplets['relationship_id'] == 'r5']['tail_id'])]['entity_type'] == 'flavor').all())

def meta_data_exists(entities, triplets, meta):
    meta_ids = triplets[triplets['relationship_id'] == 'r5']['metadata_ids'].apply(lambda x: ast.literal_eval(x)[0])
