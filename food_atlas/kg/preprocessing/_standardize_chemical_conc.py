...
from .constants._units import UNITS, UNITS_ENERGY, UNITS_MASS, UNITS_MOLE, UNITS_VOLUME
...
def parse_conc_string(row: pd.Series) -> tuple[float, str, str]:
    ...
    if "-" in conc_str:
        ...
        if len(conc_terms) == 2:
            ...
            conc_value_first, conc_unit_first = _separate_conc_value_and_unit(
                conc_first
            )
            conc_value_second, conc_unit_second = _separate_conc_value_and_unit(
                conc_second
            )
...
def convert_conc_unit(row: pd.Series) -> tuple[float, str]:
    ...
    if len(_conc_unit_terms) == 1:
        ...
        _conc_unit_factor_num, _conc_unit_num = _separate_unit_and_factor(
            _conc_unit_terms[0]
        )
        if _conc_unit_factor_num:
            raise ValueError(f"Invalid conc_unit: {row['_conc_unit']}")

        _conc_unit_factor_den, _conc_unit_den = _separate_unit_and_factor(
            _conc_unit_terms[1]
        )
...
def standardize_chemical_conc(metadata: pd.DataFrame) -> pd.DataFrame:
    ...
    metadata[["conc_value", "conc_unit"]] = metadata.progress_apply(
        convert_conc_unit, axis=1
    ).apply(pd.Series)
