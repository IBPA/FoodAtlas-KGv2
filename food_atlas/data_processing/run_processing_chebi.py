...
if __name__ == "__main__":
    ...
    for _, row in chebi_3star.iterrows():
        if row["NAME"] not in lut_chemical:
            lut_chemical[row["NAME"]] = [row.name]
    # - Synonyms.
    chebi_synonyms_3star = chebi_synonyms[
        chebi_synonyms["COMPOUND_ID"].isin(chebi_3star.index)
    ]
    ...
    lut_chemical = {
        k: list(OrderedDict.fromkeys(v).keys()) for k, v in lut_chemical.items()
    }
    ...
    for _, row in chebi_2star.iterrows():
        if row["NAME"] not in lut_chemical:
            lut_chemical[row["NAME"]] = [row.name]
    # - Synonyms.
    chebi_synonyms_2star = chebi_synonyms[
        chebi_synonyms["COMPOUND_ID"].isin(chebi_2star.index)
    ]
